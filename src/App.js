...
function DynamicModel({ modelConfig }) {
  ...
  return <primitive object={scene} scale={modelConfig.scale} position={modelConfig.position} rotation={modelConfig.rotate} />;
}
...
function SceneContent({ selectedModelId, isLocked, setIsLocked }) {
  ...
  return (
    <>
      ...
      {/* Texto principal */}
      <Text position={modelConfig.textPosition} rotation={modelConfig.textRotation} fontSize={20} font="/fonts/go3v2.ttf" anchorX="center" anchorY="middle">
        <meshStandardMaterial color={modelConfig.textColor} emissive={modelConfig.textColor} emissiveIntensity={0.4} metalness={0.3} roughness={0.4} />
        {modelConfig.textContent}
      </Text>

      {/* Easter egg */}
      <Text position={[0, 0, 0]} rotation={[0, Math.PI / 2, 0]} fontSize={0.2} font="/fonts/enchantedland.otf" anchorX="center" anchorY="middle">
        <meshStandardMaterial color="#ff0044" emissive="#ff0044" emissiveIntensity={0.2} metalness={0.4} roughness={0.6} />
  ...
  return (
    <div
      ...
      <Canvas shadows camera={{ position: [300, 0, 0], fov: 50 }} dpr={[1, 1.5]}>
        <Suspense fallback={<Loader />}>{isChangingModel ? <Loader /> : <SceneContent selectedModelId={selectedModel} isLocked={isLocked} setIsLocked={setIsLocked} />}</Suspense>
      </Canvas>
